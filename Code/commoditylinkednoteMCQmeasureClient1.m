function [meanreturn,meanexcessreturn,stdexcessreturn,fairvalue,sharpe, CI , elapsedTime, discpayoffs] = commoditylinkednoteMCQmeasureClient1 ...
...
discpayoffs= exp(-r.*T).*1000000.*max((1/3).*min((SCuT)./Cu_P,1.5)...
                    +(1/3).*min((SAlT)./Al_P,1.5)...
                    +(1/3).*min((SZnT)./Zn_P,1.5),1.06);
